...
export default function Tokenz({ publicKey, connected, signTransaction, connection, onTokenCreationSuccess }) {
  ...
  const createToken = async (tokenData) => {
    ...
    try {
      ...
      try {
        ...
        // Handle specific blockhash errors
        if (sendError.message.includes('Blockhash not found') || 
          ...
          const retryResponse = await fetch('https://pumpportal.fun/api/trade-local', {
            ...
            body: JSON.stringify({
              ...
              amount: parseFloat(tokenData.devBuyAmount) || 0.1,
              slippage: 10,
              priorityFee: 0.001, // Slightly higher priority fee for retry
              pool: 'pump'
            }),
          });
  ...
  return (
    <>
      <div className="bg-white rounded-xl p-6 border border-gray-200 mb-2 max-w-2xl shadow-md">
      ...
      <div className='flex justify-center mb-4 text-sm underline text-zinc-400 cursor-pointer'>or import an existing token</div>
